...
test('CSV has a header and one row per hit', () => {
  ...
  assert.strictEqual(lines[0], 'x,z,nearby_structures,seed,mc_version');
  assert.strictEqual(lines.length, 3);
  assert.strictEqual(lines[1], '128,-256,3,141,1.21');
  assert.strictEqual(lines[2], '-1024,512,0,141,1.21');
});
...
test('csvField quotes only when needed', () => {
  ...
  assert.strictEqual(csvField('line\nbreak'), '"line\nbreak"');
});

test('JSON export round-trips with world, criteria and results', () => {
  const parsed = JSON.parse(resultsToJSON(HITS, META));
  assert.strictEqual(parsed.seed, '141');
  assert.strictEqual(parsed.mcVersion, '1.21');
  assert.strictEqual(parsed.largeBiomes, false);
  assert.deepStrictEqual(parsed.criteria, { mainBiomes: [185] });
