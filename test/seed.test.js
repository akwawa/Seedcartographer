...
test('numeric seeds pass through as 64-bit integers', () => {
  ...
  assert.strictEqual(seedToBigInt('0'), 0n);
  assert.strictEqual(seedToBigInt('-1'), -1n);
  assert.strictEqual(seedToBigInt('  42  '), 42n, 'whitespace is trimmed');
});

test('numeric seeds are sign-extended / wrapped to 64 bits like Java long', () => {
  assert.strictEqual(seedToBigInt('9223372036854775807'), 9223372036854775807n);
  // 2^63 overflows a Java long and wraps to its minimum value
  assert.strictEqual(seedToBigInt('9223372036854775808'), -9223372036854775808n);
  // 2^64 wraps to 0
  assert.strictEqual(seedToBigInt('18446744073709551616'), 0n);
});

test('text seeds use Java String.hashCode', () => {
  // Reference values from java.lang.String#hashCode
  assert.strictEqual(seedToBigInt('abc'), 96354n);
  assert.strictEqual(seedToBigInt('a'), 97n);
  assert.strictEqual(seedToBigInt(''), 0n);
  assert.strictEqual(seedToBigInt('Glacier'), BigInt('Glacier'.split('').reduce((h, c) => (Math.imul(31, h) + c.charCodeAt(0)) | 0, 0)));
});
...
test('non-numeric-looking strings are hashed, not parsed', () => {
  ...
  assert.notStrictEqual(seedToBigInt('12e3'), 12000n);
